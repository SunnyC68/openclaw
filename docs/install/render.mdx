  ...
  - type: web
    ...
    envVars:
      ...
      - key: OPENCLAW_GATEWAY_PORT
        value: "8080" # must match PORT so the gateway listens where Render expects
      - key: SETUP_PASSWORD
...
The Docker entrypoint automatically seeds a config with `bind: "lan"` when
`OPENCLAW_STATE_DIR` is set, so the gateway listens on `0.0.0.0` instead of
loopback. This is required for Render's proxy to reach the container.
...
| `OPENCLAW_GATEWAY_PORT`  | Tells the gateway which port to bind (Render's `PORT` env var is not read by the gateway) |
...
- Port mismatch — `OPENCLAW_GATEWAY_PORT` must match `PORT` (both `8080`). The gateway does not read the `PORT` env var; it reads `OPENCLAW_GATEWAY_PORT`.

### Health check timeout (port scanning fails)

If the logs show `listening on ws://127.0.0.1:8080` and Render keeps saying "Continuing to scan for open port 8080", the gateway is binding to loopback instead of all interfaces. Render's proxy cannot reach `127.0.0.1` inside the container.

The Docker entrypoint seeds a config with `bind: "lan"` automatically when `OPENCLAW_STATE_DIR` is set. If you deployed before this entrypoint was added and already have a config file, open **Dashboard → your service → Shell** and either delete the old config to let the entrypoint re-seed it:

```
rm /data/.openclaw/openclaw.json
```

Or manually add the bind setting to your existing config:

```
node -e "const f='/data/.openclaw/openclaw.json',c=JSON.parse(require('fs').readFileSync(f));c.gateway=c.gateway||{};c.gateway.bind='lan';c.gateway.controlUi=c.gateway.controlUi||{};c.gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true;require('fs').writeFileSync(f,JSON.stringify(c,null,2))"
```

Then trigger a redeploy from the Dashboard.
